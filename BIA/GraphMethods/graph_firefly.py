...
from BIA.ex_9_firefly.firefly_repr import Swarm
from BIA.function import OptimizationFunction


def graph_firefly_optimization(optimization_function: Type['OptimizationFunction'],
    ...
    particles_3d = ax3d.scatter([], [], [], color='orange', s=50, label='Fireflies')
    best_3d = ax3d.scatter([], [], [], color='red', s=100, label='Best Firefly')

    particles_2d = ax2d.scatter([], [], color='orange', s=50, label='Fireflies')
    best_2d = ax2d.scatter([], [], color='red', s=100, label='Best Firefly')
    ...
    anim.save(f'Outputs/firefly_optimization_is{init_seed}_ss{solution_seed}-{optimization_function.__name__}.gif',
              writer='pillow', fps=1)
